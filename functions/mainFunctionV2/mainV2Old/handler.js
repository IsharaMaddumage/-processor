...
module.exports.handler = function(event, context, cb) {
  var queue = new SqsQueueParallel({
      name: "main_notification_queue_v2",
      ...
      pool.getConnection(function(err,connection){
        ...
        connection.query('SELECT * FROM ?? WHERE ?? = ?', ['engagements','id',parseInt(engagement_id)],function(err,rows){
            if(!err) {
                ...
                var startDate = new Date(engagementItem.start_date);
                var endDate = new Date(engagementItem.end_date);
                var currentDate = new Date();
                currentDate.setHours ( currentDate.getHours() + 8 );
                if(engagementItem.status == "LIVE" && startDate <= currentDate && currentDate <= endDate){
                                    ...
                                    if(!listDataErr) {
                                      ...
                                      //console.log(item.mobileuser_id);  
                                          var params = {
                                            ...
                                            QueueUrl: "https://sqs.ap-southeast-1.amazonaws.com/272397067126/SQS_Test_App_v2"
                                          };
